...
export default function FormationSelector() {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
      <Typography variant="subtitle1" sx={{ color: theme.palette.text.secondary, fontSize: '1.5rem', display: 'flex', alignItems: 'center' }}>Formation</Typography>
      <Select
        value={formation}
        onChange={handleFormationChange}
        sx={{
          backgroundColor: theme.palette.mode === 'dark' ? 'rgba(var(--color-surface-200-rgb), 0.6)' : 'rgba(0, 0, 0, 0.1)', // Use theme mode
        }}
      >
