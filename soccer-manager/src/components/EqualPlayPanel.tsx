...
export default function EqualPlayPanel() {
  ...
  return (
    <div className="space-y-3">
      <div className="text-sm text-neutral-300">Team average: <span className="font-semibold">{formatClock(stats.avg)}</span></div>
      ...
      {(suggestOut && suggestIn) && (
        <div className="rounded-lg border border-emerald-700 bg-emerald-900/10 p-3 text-sm">
          Suggestion: sub OUT <span className="font-semibold">{suggestOut.name}</span> ({formatClock(suggestOut.ms)}) and sub IN <span className="font-semibold">{suggestIn.name}</span> ({formatClock(suggestIn.ms)}).
        </div>
      )}
    </div>
  )
