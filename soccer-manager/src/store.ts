...
export const formationLayouts = {
  ...
  "3-5-2": {
    ...
    lwb: { gridArea: "lwb", order: 5 },
    cm1: { gridArea: "cm1", order: 6 },
    cm2: { gridArea: "cm2", order: 7 },
    cm3: { gridArea: "cm3", order: 8 },
    rwb: { gridArea: "rwb", order: 9 },
    st1: { gridArea: "st1", order: 10 },
    st2: { gridArea: "st2", order: 11 },
  },
  "4-4-2 (Diamond)": {
...
export const useAppStore = create<AppStore>()(
  persist(
    (set, get) => ({
      ...
      pauseClock: () =>
        set((s) => {
          if (!s.isRunning || s.clock.startedAtSec == null) {
            return s;
          }
          const now = Date.now() / 1000;
          ...
          if (s.isRunning && s.clock.startedAtSec != null) {
            const elapsedSinceGameClockStart = now - s.clock.startedAtSec;
      ...
      setGameClockTime: (newTimeInSeconds) =>
        set((s) => {
          ...
          // First, accumulate current live minutes for all players on field
          if (s.isRunning) {
            const elapsedSinceGameClockStart =
              now - (s.clock.startedAtSec ? s.clock.startedAtSec : 0);
            updatedRoster = s.roster.map((pl) =>
              pl.isOnField
                ? {
                    ...pl,
